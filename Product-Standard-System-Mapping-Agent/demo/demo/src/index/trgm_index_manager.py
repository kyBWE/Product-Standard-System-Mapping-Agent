...
from src.models.index_result import TrgmSearchResult
    ...
    def insert_category_texts(self, nodes: list[CategoryNode]) -> int:
        ...
        for node in nodes:
            try:
                ...
                self._db.execute(
                    """INSERT INTO category_texts (category_id, category_name, syn_list, category_pids, category_group_name)
                       VALUES (%s, %s, %s, %s, %s)
                ...
                logger.warning(f"写入文本数据异常: category_id={node.category_id}, error={e}")
