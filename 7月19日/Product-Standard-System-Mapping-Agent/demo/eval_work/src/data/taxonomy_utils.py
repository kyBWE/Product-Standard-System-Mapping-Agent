...
def suggest_expansion_path(
    ...
    *,
    ...
    root_nodes = page_tree.get_root_nodes()

    index_hits = page_tree.lookup_index(product_name, top_k=top_roots)
    if index_hits:
        top_root_ids = {hit.root.category_id for hit in index_hits}
        candidates = []
        for r in root_nodes:
            if r.category_id in top_root_ids:
                candidates.append((r.category_id, r.category_name))
        if not candidates:
            candidates = [(r.category_id, r.category_name) for r in root_nodes]
    else:
        candidates = [(r.category_id, r.category_name) for r in root_nodes]
    ...
    for depth in range(max_depth):
        ...
        level_hint = "一级分类" if depth == 0 else f"第{depth + 1}层子分类"
        current_path_str = " > ".join(current_path_names) if current_path_names else ""

        analysis = llm.detailed_category_analysis(
        ...
        parent_id = picked_id
        current_path_names.append(node.category_name)
        path_nodes.append({
            "level": depth + 1,
            "category_id": picked_id,
    ...
    suggested_path = analysis.get("suggested_path", [])
    if not suggested_path:
        suggested_path = [analysis.get("suggested_category_name", product_name)]

    last_existing_name = path_nodes[-1]["category_name"] if path_nodes else ""
    for i, new_name in enumerate(suggested_path):
        if new_name == last_existing_name and i == 0:
            continue
        path_nodes.append({
    ...
    suggested_name = suggested_path[-1] if suggested_path else product_name
    confidence = analysis.get("confidence", 0)
    llm_reason = analysis.get("reason", "")

    sibling_nodes = []
    if parent_id:
        parent_node = page_tree.get_node(parent_id)
        if parent_node:
    ...
    full_path_text = " > ".join(
        n["category_name"] for n in path_nodes
    )

    return {
        "product_name": product_name,
        "path": path_nodes,
        "path_text": full_path_text,
