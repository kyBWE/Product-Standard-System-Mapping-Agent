...
def init_engines(config_path: str = "config.yaml"):
    ...
    page_engine_force = PageIndexEngine(page_tree, llm, force_llm_each_layer=True, vec_mgr=vec_mgr, rerank=rerank_adapter, trgm_mgr=trgm_mgr)
...
def evaluate(limit=0):
    ...
    def process_item(item):
        ...
        for ek, engine in engine_map.items():
            t0 = time.time()
            try:
                logger.info(f"[DEBUG] 开始引擎 {ek}: {item['product_name'][:30]}")
                result = engine.match(item["product_name"])
                elapsed = time.time() - t0
                predicted = result.matched_category_id
                conf = result.confidence
                logger.info(f"[DEBUG] 完成引擎 {ek}: {item['product_name'][:30]} -> {predicted} ({elapsed:.1f}s)")
