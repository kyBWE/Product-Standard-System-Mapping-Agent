...
EMBEDDING_DIM = API_EMBEDDING_DIM
...
class VectorIndexManager:
    def __init__(self, db: DBConnectionManager, embedding_model: str = "api",
    ...
    def _init_embedder(self) -> None:
        if self._embedding_model == "api":
            try:
                from src.index.api_embedder import ApiEmbedder
                if self._embedding_config is not None:
                    self._api_embedder = ApiEmbedder(
                        api_key=self._embedding_config.api_key,
                        base_url=self._embedding_config.base_url,
                        model=self._embedding_config.model,
                        embedding_dim=self._embedding_config.dimension,
                        max_retries=self._embedding_config.max_retries,
                        timeout=self._embedding_config.timeout,
                        batch_size=self._embedding_config.batch_size,
                    )
                    self._embedding_dimension = self._embedding_config.dimension
                else:
                    api_key = self._api_key or ""
                    base_url = self._base_url or "https://api.modelarts-maas.com/v1/embeddings"
                    self._api_embedder = ApiEmbedder(
                        api_key=api_key,
                        base_url=base_url,
                        model="bge-m3",
                        embedding_dim=self._embedding_dimension or API_EMBEDDING_DIM,
                    )
                    self._embedding_dimension = self._embedding_dimension or API_EMBEDDING_DIM
                self._use_api = True
                logger.info(f"API embedder initialized, dim={self._embedding_dimension}, model=bge-m3")
            except Exception as e:
                logger.warning(f"API embedder init failed: {e}, falling back to TF-IDF")
                self._use_api = False
        elif self._embedding_model == "onnx":
            try:
                from src.index.onnx_embedder import ONNXEmbedder
                self._onnx_embedder = ONNXEmbedder()
                self._use_onnx = True
                self._embedding_dimension = ONNX_EMBEDDING_DIM
    ...
    def ensure_pgvector_ready(self) -> bool:
        """尝试启用 pgvector 扩展、补齐向量列并创建 HNSW 索引。"""
        ...
        vec_col = self._get_vec_column()
        if not self._has_vec_column(vec_col):
            if vec_col == "vec_bgem3":
                logger.info(f"vec_bgem3 列不存在，跳过创建（假设已由数据导入创建）")
                if self._has_vec_column("vec_bgem3"):
                    pass
                else:
                    self._use_pgvector = False
                    return False
                ...
                try:
                    self._db.execute(
                        ...
                        f"vec_search vector({self._embedding_dimension or EMBEDDING_DIM})"
                    )
    ...
    def _has_vec_column(self, col_name: str) -> bool:
        try:
            rows = self._db.execute(
                """SELECT 1 FROM information_schema.columns
                   WHERE table_name = 'category_vectors' AND column_name = %s""",
                (col_name,),
            )
            return bool(rows)
        except Exception:
    ...
    def _ensure_hnsw_index(self) -> None:
        if not self._use_pgvector:
            return
        vec_col = self._get_vec_column()
        idx_name = f"idx_category_vectors_{vec_col}"
        try:
            self._db.execute(
                f"""CREATE INDEX IF NOT EXISTS {idx_name}
                   ON category_vectors USING hnsw ({vec_col} vector_cosine_ops)"""
            )
            logger.info(f"HNSW向量索引已就绪 ({vec_col})")
        except Exception as e:
    ...
    def invalidate_matrix(self) -> None:
        ...
        self._matrix_category_ids = []
        self._matrix_category_names = []
        self._category_ids = []
        self._category_names = []
        logger.info("内存向量矩阵已失效，下次查询时从数据库重新加载")
        ...
        if self._use_pgvector:
            self._db.execute(f"""
                CREATE TABLE IF NOT EXISTS category_vectors (
                    ...
                    embedding       BYTEA,
                    vec_search      vector({self._embedding_dimension or EMBEDDING_DIM}),
    ...
    def insert_category_vectors(self, nodes: list[CategoryNode]) -> int:
        ...
        if self._use_api:
            return self._insert_api_vectors(nodes)
        elif self._use_onnx:
            return self._insert_onnx_vectors(nodes)
        else:
            return self._insert_tfidf_vectors(nodes)

    def _build_embedding_text(self, node: CategoryNode) -> str:
        parts = [node.category_name]
        if node.syn_list:
            parts.extend(node.syn_list)
        if node.category_group_name:
            parts.append(node.category_group_name)
        return " ".join(parts)

    def _insert_api_vectors(self, nodes: list[CategoryNode]) -> int:
        texts = []
        for node in nodes:
            texts.append(self._build_embedding_text(node))
        ...
        for chunk_start in range(0, total, chunk_size):
            ...
            try:
                embeddings = self._api_embedder.embed_batch(chunk_texts)
            except Exception as e:
                logger.warning(f"API embedding batch failed (start={chunk_start}): {e}")
                embeddings = [np.zeros(self._embedding_dimension, dtype=np.float32)] * len(chunk_texts)
            ...
            logger.info(f"API向量写入进度: {chunk_end}/{total}")
        ...
        logger.info(f"API向量写入完成: 成功{success}/{total}条")
        return success
    ...
    def update_category_vectors(self, nodes: list[CategoryNode]) -> int:
        ...
        for node in nodes:
            try:
                if self._use_api and self._api_embedder:
                    text = self._build_embedding_text(node)
                    vec = self._api_embedder.embed(text)
                elif self._use_onnx:
                    text = self._build_embedding_text(node)
    ...
    def _get_vec_column(self) -> str:
        if self._embedding_dimension == 1024:
            return "vec_bgem3"
        return "vec_search"
    ...
    def _search_pgvector(self, query_vector: list[float], top_k: int = 10) -> list[VectorSearchResult]:
        ...
        try:
            rows = self._db.execute(
                f"""SELECT category_id, category_name, 1 - ({vec_col} <=> %s::vector) AS similarity
                   FROM category_vectors
                   WHERE {vec_col} IS NOT NULL
                   ORDER BY {vec_col} <=> %s::vector
                   LIMIT %s""",
    ...
    def _load_vectors_from_db(self) -> None:
        try:
            vec_col = self._get_vec_column()
            if self._use_api or self._embedding_dimension == 1024:
                rows = self._db.execute(
                    f"SELECT category_id, category_name, {vec_col} AS embedding FROM category_vectors WHERE {vec_col} IS NOT NULL"
                )
            else:
            ...
            for r in rows:
                ...
                elif isinstance(emb, memoryview):
                    vec = pickle.loads(bytes(emb))
                elif isinstance(emb, bytes):
                    vec = pickle.loads(emb)
                else:
