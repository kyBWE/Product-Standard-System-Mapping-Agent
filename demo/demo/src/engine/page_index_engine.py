...
class PathState:
    ...
    def score(self) -> float:
        depth_score = min(len(self.path) / 10.0, 1.0) * 0.35
        hit_score = min(self.rule_hits / max(len(self.path), 1), 1.0) * 0.35
        guide_score = min(self.index_guided_steps / max(len(self.path), 1), 1.0) * 0.15
        llm_penalty = max(0.0, 1.0 - self.llm_steps * 0.08) * 0.15
        return depth_score + hit_score + guide_score + llm_penalty
...
class PageIndexEngine:
    ...
    def _compute_confidence(
        ...
        if self._force_llm_each_layer:
            ...
            return max(base, 0.45 + llm_ratio * 0.35)
