...
def judge_file(
    ...
    if limit > 0:
        need = need[:limit]
    logger.info(f"{path.name}: soft未命中待判定 {len(need)} 条")

    ok_count = 0
    t0 = time.time()
    for n, idx in enumerate(need, start=1):
        row = details[idx]
        ...
        reasonable, conf, reason = llm.judge_mapping_reasonable(
        ...
        if n % 10 == 0 or n == len(need):
            logger.info(
                f"  [{n}/{len(need)}] llm_accept={ok_count} "
                f"latest={row['product_name']} -> {pred_name} "
                f"ok={reasonable} ({reason[:40]})"
            )

    # 已 soft 命中的也标 llm_soft
    for row in details:
    ...
    if "llm200" in path.name:
        out = Path("output/eval_llm200_full_llm_soft.json")
    elif "llmv2" in path.name:
        out = Path("output/eval_llmv2_full_llm_soft.json")
...
def main():
    ...
    cmp = Path("output/eval_full_compare_llm_soft.json")
    cmp.write_text(
